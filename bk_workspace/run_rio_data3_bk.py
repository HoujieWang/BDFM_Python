...
nSample = 1000
pois_samples = np.zeros((nSample, TActual, N))
bern_samples = np.zeros((nSample, TActual, N))
st = time.time()
for n in range(N):
    # n = 0
    print(n)
    ...
    bern_samples[:,:, n] = RA_prob
    pois_samples[:,:, n] = RA_rate

bern_mean = np.mean(bern_samples, 0)
pois_mean = np.mean(pois_samples, 0)